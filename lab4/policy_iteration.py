...
#        self.actions_variants = np.array([0,1,2,3])
        ...
        self.policy_probs = np.full((self.observation_dim, self.actions_variants), 0.25)
    ...
    def policy_improvement(self):
        ...
        Улучшение стратегии 
        '''
        qvaluesMatrix=np.zeros((self.observation_dim, self.actions_variants))
        improvedPolicy=np.zeros((self.observation_dim, self.actions_variants))
        # Цикл по состояниям
        for state in range(self.observation_dim):
